...
let shippingAddress: string = "(323) 242-8254 9400 S Normandie Ave #14 Los Angeles, California(CA), 90044";
let productName: string = products[2].name;
let product = products.find(item => item.name === productName);
let originalProductPrice: number = Number(product && product.price)
// setting billing variables based on conditions:
if(Number(product && product.price) > 25) shipping = 0;
if(shippingAddress.match(pattern)) taxPercent = 0.1;
// 
let calc = (Number(product && product.price) * taxPercent);
let taxTotal: number = calc;
if(product) total = Number(product && product.price) + shipping + taxTotal;
